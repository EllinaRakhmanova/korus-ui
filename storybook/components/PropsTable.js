...
const getFlowTypes = (src, flowType) => {
...
const getPropTypes = (props, source) => {
...
const addTypesToProps = ({ source, context, props }) => {
...
const renderPropTables = (propTypes) => {
  ...
  const sortProps = (props) => props.sort((firstEl, secondEl) => {
  ...
  if (propTypes[0].componentName && Array.isArray(propTypes)) {
    return propTypes.map((component) => {
